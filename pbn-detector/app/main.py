...
def _risk_level(prob: float, thresholds: Optional[dict[str, float]] = None) -> str:
    """Determine risk level using adaptive thresholds if enabled"""
    if thresholds:
...
async def detect(request: BacklinkDetectionRequest) -> BacklinkDetectionResponse:
    try:
        ...
        # Precompute network features ONCE for all backlinks - O(n) instead of O(n²)
        ...
        # Calculate network-level content similarity (once for all backlinks)
        try:
            snippets = [
        ...
        # Extract enhanced features for all backlinks (once)
        ...
        # Process backlinks in parallel if enabled and dataset is large enough
        summary = DetectionSummary()
...
async def _process_backlinks_sequential(
    ...
    """Process backlinks sequentially (original method)"""
    items: list[DetectionItem] = []
    
    for backlink in peers:
        try:
            # Use precomputed network features - O(1) instead of O(n)
            ...
            # Apply enhanced features boost
            if settings.use_enhanced_features:
                enhanced_features = enhanced_features_map.get(backlink.source_url, {})
                if enhanced_features:
                    # Boost probability based on enhanced features
                    if enhanced_features.get('link_stability', 0.5) > 0.6:
                        probability += 0.1  # Short-lived links are riskier
                    if enhanced_features.get('temporal_clustering', 0) > 0.5:
                        probability += 0.15  # Burst patterns are PBN indicator
                    if enhanced_features.get('clustering_coefficient', 0) > 0.5:
                        probability += 0.1  # High clustering = PBN
                    if enhanced_features.get('rank_z_score', 0) > 0.7:
                        probability += 0.1  # Outlier rank = suspicious
                    if enhanced_features.get('spam_z_score', 0) > 0.7:
                        probability += 0.1  # Outlier spam score = suspicious
                    
                    probability = min(probability, 0.99)  # Cap at 0.99
        ...
        rule_weight = 0.3
        content_weight = 0.15
        base_weight = 1.0 - rule_weight - content_weight  # 0.55
        
        # Normalize rule boost: cap at 1.0
        normalized_rule_boost = min(rules_boost, 1.0)
        
        # For high-risk backlinks (high spam score or very low rank), increase rule weight
        is_high_risk_signal = (
        ...
        if is_high_risk_signal:
            # Increase rule weight for high-risk signals
            rule_weight = 0.4
            base_weight = 1.0 - rule_weight - content_weight  # 0.45
        ...
        # Direct boost for high-risk signals (high spam score + low rank)
        if is_high_risk_signal and normalized_rule_boost > 0:
            # Additional boost when both spam score and domain quality rules trigger
            if "dataforseo_spam_score" in rule_scores and "domain_quality" in rule_scores:
                boosted_probability += 0.25  # Significant boost for combined high-risk signals
            elif normalized_rule_boost >= 0.3:
                boosted_probability += 0.15  # Moderate boost for high rule scores
...
async def _process_backlink_single(
    ...
    """Process a single backlink (for parallel processing)"""
    try:
        features = feature_extractor.build_feature_vector(backlink, network_features)
        probability = classifier_service.predict_proba(features, backlink)
...
async def _process_backlinks_parallel(
    ...
    """Process backlinks in parallel using asyncio"""
    items: list[DetectionItem] = []
