...
class CitationService
{
    ...
    public function computeCompetitors(array $byQuery, string $targetUrl): array
    {
        ...
        $targetDomainVariations = $this->getDomainVariations($targetDomain);
        $competitorData = [];
        $totalQueries = count($byQuery);
        $totalCitations = 0;

        foreach ($byQuery as $queryIndex => $entry) {
            $query = $entry['query'] ?? '';
            foreach (['gpt', 'gemini'] as $provider) {
                $refs = $entry[$provider]['citation_references'] ?? [];
                $citationFound = $entry[$provider]['citation_found'] ?? false;
                
                if ($citationFound && !empty($refs)) {
                    foreach ($refs as $url) {
                        ...
                        $competitorData[$domain]['citation_count']++;
                        $totalCitations++;
                        
                        if (!in_array($query, $competitorData[$domain]['queries'])) {
                            $competitorData[$domain]['query_count']++;
                            $competitorData[$domain]['queries'][] = $query;
                        }

                        if (!in_array($url, $competitorData[$domain]['urls'])) {
        ...
        foreach ($competitorData as &$competitor) {
            $competitor['percentage'] = $totalCitations > 0 
                ? round(($competitor['citation_count'] / $totalCitations) * 100, 2)
                : 0;
    ...
    protected function getDomainVariations(string $domain): array
    {
        $variations = [$domain];
        
        if (strpos($domain, 'cricinfo') !== false) {
        ...
        if ($baseDomain !== $domain && $baseDomain) {
            $variations[] = $baseDomain;
        }
        
        return array_unique($variations);
    }

    protected function isTargetDomain(string $domain, array $targetVariations): bool
    {
        $normalizedDomain = $this->extractDomain($domain);
        foreach ($targetVariations as $variation) {
            $normalizedVariation = $this->extractDomain($variation);
            if ($normalizedDomain === $normalizedVariation) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Generate generic, non-brand-focused queries based on domain topic
     * Used as fallback when LLM doesn't generate enough queries
     */
    ...
    /**
     * @deprecated This method generates brand-focused queries which don't match requirements
